"""
Amanz Web Scraper for Malaysian Phone Database
Scrapes phone data with MYR pricing from Amanz
"""
import requests
...
class AmanzScraper:
    """
    Scraper for Amanz Malaysia
    Website: https://amanz.my/
    """

    def __init__(self):
        self.base_url = "https://amanz.my"
        self.headers = {
    ...
    def scrape_phone_articles(self, max_phones=50):
        """
        Scrape phone information from Amanz articles
        ...
        """
        print(f"\n[INFO] Scraping phone articles from Amanz...")

        phones_data = []

        # Amanz categories and tags
        categories = [
            'smartphones',
            'telefon-pintar',
            'gadget',
        ...
        for category in categories:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if not articles:
                        articles = soup.find_all(['div'], class_=re.compile(r'post|entry|card'))

                    print(f"[INFO] Found {len(articles)} articles in '{category}'")

                    for article in articles[:10]:  # Process up to 10 articles per category
                        try:
                            # Find article link
                            ...
                            if link:
                                ...
                                # Only process if it looks like a phone article
                                if self._is_phone_article(article_url, article.get_text()):
                    ...
                    print(f"[WARN] Could not access category (status {response.status_code})")

            except Exception as e:
                print(f"[ERROR] Failed to scrape '{category}': {str(e)}")
                continue

        print(f"\n[INFO] Total phones scraped: {len(phones_data)}")
        return phones_data

    def _is_phone_article(self, url, text):
        """Check if article is about a phone"""
        phone_keywords = ['phone', 'smartphone', 'galaxy', 'iphone', 'xiaomi',
                          'oppo', 'vivo', 'realme', 'honor', 'launched',
                          'review', 'specs', 'specification', 'harga']

        url_lower = url.lower()
        text_lower = text.lower()

        return any(keyword in url_lower or keyword in text_lower for keyword in phone_keywords)
        ...
        try:
            ...
            if content:
                ...
                price_patterns = [
                    r'RM\s*(\d+[,\d]*)',
                    r'harga\s+RM\s*(\d+[,\d]*)',
                    r'berharga\s+RM\s*(\d+[,\d]*)',
                    r'dijual\s+pada\s+harga\s+RM\s*(\d+[,\d]*)',
                ...
                # Storage
                storage_match = re.search(r'(\d+)\s*GB\s+(?:of\s+)?(?:storage|simpanan|internal)', content_text, re.IGNORECASE)
                if storage_match:
                    phone_data['storage'] = f"{storage_match.group(1)}GB"

                # Camera (main sensor)
                camera_patterns = [
                    r'(\d+)\s*MP\s+(?:main|primary|rear|utama)',
                    r'(?:main|primary|utama)\s+(\d+)\s*MP',
                ]
    ...
    def _extract_model_name(self, text):
        ...
        # Remove common prefixes (English and Malay)
        text = re.sub(r'(?:now|official|announced|launched|available|review|hands-on|first look|dilancarkan|rasmi|dijual)[:|-]?\s*', '', text, flags=re.IGNORECASE)
        text = re.sub(r'\s+(?:in|di)\s+Malaysia.*', '', text, flags=re.IGNORECASE)
        text = re.sub(r'\s*-\s*Harga.*', '', text, flags=re.IGNORECASE)
    ...
    def _extract_brand(self, text):
        ...
        brands = ['Samsung', 'Apple', 'Xiaomi', 'Oppo', 'Vivo', 'Realme',
                  ...
                  'Sony', 'Google', 'Nothing', 'Infinix', 'Tecno', 'Lenovo']
...
def main():
    ...
    print(f"\nStarting Amanz scraper (limit: {limit} phones)...")
    print("This may take several minutes...\n")

    print("="*60)
    print("Amanz Phone Scraper for DialSmart")
    print("="*60)
