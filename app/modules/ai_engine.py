...
class AIRecommendationEngine:
    ...
    def get_phones_by_battery(self, budget_range=None, brand_ids=None, top_n=5):
        """Get phones sorted by battery capacity for long-lasting queries"""
        query = Phone.query.filter_by(is_active=True)
        ...
        for phone in phones:
            ...
            if specs and specs.battery_capacity:
                # Parse battery capacity (handle formats like "5000mAh" or "5000")
                try:
                    battery = int(str(specs.battery_capacity).replace('mAh', '').replace('mah', '').strip())
                    results.append({
                        'phone': phone,
                        'specifications': specs,
        ...
        # Sort by battery capacity (highest first)
        results.sort(key=lambda x: x['battery_capacity'], reverse=True)
        return results[:top_n]

    def get_phones_by_specs(self, criteria, top_n=5):
        """Get phones matching specific specification criteria (5G, AMOLED, etc.)"""
        query = Phone.query.filter_by(is_active=True)

        # Apply budget filter if specified
        if criteria and 'min_budget' in criteria and 'max_budget' in criteria:
            query = query.filter(Phone.price >= criteria['min_budget'], Phone.price <= criteria['max_budget'])
        ...
        for phone in phones:
            ...
            # Check if phone matches criteria
            matches = True

            # Check 5G requirement
            if criteria and criteria.get('requires_5g'):
                if not specs.network_5g or specs.network_5g.lower() not in ['yes', 'true', '5g']:
                    matches = False

            # Check RAM requirement
            if criteria and 'min_ram' in criteria:
                ram_values = extract_ram_values(specs.ram_options)
                if not ram_values or max(ram_values) < criteria['min_ram']:
                    matches = False

            # Check storage requirement
            if criteria and 'min_storage' in criteria:
                storage_str = str(specs.internal_storage) if specs.internal_storage else ""
                storage_values = []
                for s in storage_str.replace('/', ',').split(','):
                    s = s.strip().replace('GB', '').replace('gb', '').strip()
                    if s.isdigit():
                        storage_values.append(int(s))
                if not storage_values or max(storage_values) < criteria['min_storage']:
                    matches = False

            # Check camera requirement
            if criteria and 'min_camera' in criteria:
                try:
                    camera = int(str(specs.rear_camera_main).replace('MP', '').replace('mp', '').strip()) if specs.rear_camera_main else 0
                    if camera < criteria['min_camera']:
                        matches = False
                except (ValueError, AttributeError):
                    matches = False
